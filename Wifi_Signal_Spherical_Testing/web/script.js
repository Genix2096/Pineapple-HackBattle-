/*
WiFi Heatmap Visualizer - Optimized Version
- Uses Python backend endpoints to get Wi-Fi scans
- Optimized canvas rendering with Level of Detail (LOD) for smooth panning and zooming.
- Decoupled data fetching from the render loop for better performance.
- Networks are positioned radially based on live RSSI, with a repulsion algorithm to prevent overlap.
...
// Network data caches
let liveNetworks = [];
let displayNodes = []; // Pre-calculated positions for rendering
...
// --- Data Handling and Logic Update ---
async function updateLogic() {
    // Fetch trilaterated AP positions from server
    let data = null;
    try {
    ...
    if(!data || !data.ok){
        // Not enough clients yet; show message and clear nodes
        displayNodes = [];
        if(result){
            result.innerHTML = 'Waiting for 3 active clients to start triangulating...';
    ...
    const aps = data.aps || [];
    const grid = data.grid || {width: 30, height: 30};
    const centerX = grid.width/2;
    const centerY = grid.height/2;
    // Scale meters to pixels to fit nicely into canvas
    const scale = Math.min(W, H) * 0.8 / Math.max(grid.width, grid.height);

    const tempNodes = aps.map(ap => {
        const x = (ap.x - centerX) * scale;
        const y = (ap.y - centerY) * scale;
        const rssi = (typeof ap.rssi_avg === 'number') ? ap.rssi_avg : -60;
        const dist = (typeof ap.distance === 'number') ? ap.distance : Math.sqrt((ap.x-centerX)**2 + (ap.y-centerY)**2);
        const meta = {ssid: ap.ssid, bssid: ap.bssid, band: ap.band, distance: dist, bearing_deg: ap.bearing_deg};
        return { id: ap.bssid, x, y, rssi, dist, meta };
    });

    displayNodes = tempNodes;
}
...
function drawGrid(){
  ...
  const step = 40;
  const startX = -panX/zoom - (W/2)/zoom;
  const startY = -panY/zoom - (H/2)/zoom;
...
function drawHotspot(node){
  ...
  const LOD_THRESHOLD = 0.6; // Zoom level to switch between detailed and simple rendering

  ctx.fillStyle = rssiToColor(rssi);
  ctx.beginPath(); ctx.arc(x, y, 6, 0, Math.PI*2); ctx.fill();

  // Only draw expensive details if zoomed in enough
...
canvas.addEventListener('mousemove', e => {
    ...
    if (zoom > 0.6) { // Only show tooltips when reasonably zoomed in
        for (const node of displayNodes) {
...
btnSuggest.addEventListener('click', async ()=>{
  ...
  function step(){
    ...
    if(t < DURATION){
      setTimeout(step, 100); // Check every 100ms
    }else{
...
btnReset.addEventListener('click', ()=>{
  ...
  updateLogic(); // Immediately re-calculate positions
});
...
(async function init(){
  ...
  setInterval(updateLogic, 2000); // Update data and logic every 2 seconds
  await updateLogic(); // Run once immediately to populate
