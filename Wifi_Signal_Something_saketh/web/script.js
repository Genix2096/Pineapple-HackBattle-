...
async function updateLogic() {
    ...
    const MIN_RADIUS = 80;
    const MAX_RADIUS = Math.min(W, H) * 0.45; // Adjusted for better spacing
    const goldenAngleIncrement = Math.PI * (3 - Math.sqrt(5));

    let tempNodes = [];
    liveNetworks.forEach(n => {
        const smooth = smoothed.get(n.bssid) || {rssi: n.rssi, dist: rssiToDistance(n.rssi, n.band)};
        const newRssi = lerp(smooth.rssi, n.rssi, 0.1);
        const newDist = lerp(smooth.dist, rssiToDistance(newRssi, n.band), 0.1);
        smoothed.set(n.bssid, {rssi: newRssi, dist: newDist});
        ...
        const rssiT = (clamp(newRssi, -90, -30) + 90) / 60;
